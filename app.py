...
class IVASSMSClient:
    ...
    def login_with_credentials(self):
        """Auto-login using IVAS_EMAIL and IVAS_PASSWORD environment variables."""
        email = os.environ.get("IVAS_EMAIL", "")
        password = os.environ.get("IVAS_PASSWORD", "")

        if not email or not password:
            logger.error("IVAS_EMAIL and IVAS_PASSWORD must be set as environment variables")
            return False
    ...
    def ensure_logged_in(self):
        if self.logged_in and self.csrf_token:
            return True
        # Try credentials first, fall back to cookies
        if os.environ.get("IVAS_EMAIL") and os.environ.get("IVAS_PASSWORD"):
...
def get_sms():
    ...
    if not result:
        return jsonify({"error": "Failed to fetch data — check IVAS_EMAIL and IVAS_PASSWORD env vars"}), 401
